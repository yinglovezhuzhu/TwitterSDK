...
import com.owen.twitter.sdk.utils.IntentUtils;
...
class SSOAuthHandler extends AuthHandler {
    ...
    private boolean startAuthActivityForResult(Activity activity) {
        ...
        if (packageName == null) {
            LogUtils.e("SSO app signature check failed", null);
            return false;
        }
        ...
        if (!IntentUtils.isActivityAvailable(activity, intent)) {
            LogUtils.e("SSO auth activity not found", null);
            return false;
        }
        ...
        try {
            ...
            LogUtils.e("SSO exception occurred" + LogUtils.getStackTrace(e));
            return false;
        }
    }
